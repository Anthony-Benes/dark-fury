...
b8 VulkanBackend::Initialize(const char* application_name, [[maybe_unused]]platform_state* plat_state) {
    context.allocator = 0;
    ...
    for (u32 i = 0; i < required_extensions.size(); i++) { Engine::Log::Debug(required_extensions[i]); }
#endif
    ...
    for (u32 i = 0; i < required_validation_layer_count; i++) {
        ...
        if (!found) {
            Engine::Log::Fatal("Required validation layer is missing: %s", required_validation_layers[i]);
            return false;
        }
    }
