...
void Window::Init(const WindowProps& props) {
    ...
    if ( !platform_startup(&mPlatform, mProps.Title, startPos.x, startPos.y, mProps.Width,
                           mProps.Height) ) {
        return;
    }
...
b8 Window::OnUpdate() { return platform_pump_message(&mPlatform); }

void Window::OnDraw(Renderer::render_packet* packet) { mRenderer->DrawFrame(packet); }

b8 application_on_event(Event::Code::System code, void* sender, void* listener_inst,
                        Event::Context context);
b8 application_on_key(Event::Code::System code, void* sender, void* listener_inst,
                      Event::Context context);
...
void Application::Run() {
    ...
    while ( mRunning ) {
        if ( !mWindow->OnUpdate() ) { mRunning = false; }
        if ( !mSuspended ) {
            mClock.Update();
...
b8 application_on_event(Event::Code::System code, [[maybe_unused]] void* sender,
                        [[maybe_unused]] void* listener_inst,
                        [[maybe_unused]] Event::Context context) {
    switch ( code ) {
...
b8 application_on_key(Event::Code::System code, [[maybe_unused]] void* sender,
                      [[maybe_unused]] void* listener_inst, Event::Context context) {
    switch ( code ) {
